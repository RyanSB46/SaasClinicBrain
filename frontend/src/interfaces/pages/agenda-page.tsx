...
export function AgendaPage() {
  ...
  const [manualMode, setManualMode] = useState<'PRESENCIAL' | 'REMOTO'>('REMOTO')
  ...
  useEffect(() => {
    const trimmed = manualPatientName.trim()
    if (trimmed.length < 2 || !patientsQuery.data) return
    const match = (patientsQuery.data ?? []).find((p) => p.name.trim().toLowerCase() === trimmed.toLowerCase())
    if (match) {
      setManualPatientPhone(formatPhoneForDisplay(match.phoneNumber))
      setManualPatientEmail(match.email ?? '')
    }
  }, [manualPatientName, patientsQuery.data])
  ...
  function handleManualAppointmentSelect(nextAppointmentId: string) {
    ...
    setManualMode(appointment.mode ?? 'REMOTO')
  ...
  function handleManualSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const payload: Parameters<typeof executeManualAppointmentAction>[0] = {
      ...
      mode: manualAction === 'BOOK' || manualAction === 'RESCHEDULE' ? manualMode : undefined,
      ...
      <article className="card">
        ...
        <form className="form-grid" onSubmit={handleManualSubmit}>
          ...
          <div>
            ...
            />
            <p className="muted-text">Obrigatório para enviar convite do Google Calendar e Meet.</p>
          </div>
              ...
              <div style={{ display: 'flex', gap: '16px', marginTop: '4px' }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer' }}>
                  <input
                    type="radio"
                    name="manual-mode"
                    checked={manualMode === 'REMOTO'}
                    onChange={() => setManualMode('REMOTO')}
                  />
                  Remoto (gera link do Google Meet)
                </label>
                <label style={{ display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer' }}>
                  <input
                    type="radio"
                    name="manual-mode"
                    checked={manualMode === 'PRESENCIAL'}
                    onChange={() => setManualMode('PRESENCIAL')}
                  />
                  Presencial
                </label>
              </div>
      ...
      {selectedAppointment && (
        <div className="modal-backdrop" onClick={() => setSelectedAppointment(null)}>
          <article className="card modal-card" onClick={(event) => event.stopPropagation()}>
            ...
            <button type="button" className="secondary-button" onClick={() => setSelectedAppointment(null)} style={{ marginTop: '12px' }}>
              Fechar
            </button>
          </article>
        </div>
