...
export function runConversationStateMachine(
  ...
  const mainGreeting = customGreeting ?? mainMenuMessage(doctorName)

  if (input.currentState === 'INITIAL') {
    return {
      nextState: 'MAIN_MENU',
      responseMessage: mainGreeting,
      shouldEnd: false,
    }
  }
  ...
  if (input.currentState === 'MAIN_MENU') {
    ...
    return {
      nextState: 'MAIN_MENU',
      responseMessage: customGreeting
        ? `Não entendi sua opção.\n${mainGreeting}`
        : invalidOptionMessage('MAIN_MENU', doctorName, bookingUrl),
      shouldEnd: false,
    }
  }
