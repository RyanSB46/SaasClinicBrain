...
import { loginStaff } from '../../../application/use-cases/auth/login-staff.use-case'
...
authRoutes.post('/auth/staff/login', validateBody(loginSchema), async (request, response, next) => {
  try {
    const result = await loginStaff(request.body)
    return response.status(200).json(result)
  } catch (error) {
    return next(error)
  }
