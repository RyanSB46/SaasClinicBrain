...
appointmentsRoutes.get('/', async (request, response, next) => {
  try {
    ...
    const mapped = appointments.map(({ googleEventMapping, ...a }) => ({
      ...a,
      meetingUrl: googleEventMapping?.meetingUrl ?? null,
    }))
  ...
  try {
    ...
    if (action === 'BOOK') {
      ...
      const created = await createAppointment({
        ...
        mode: request.body.mode === 'REMOTO' ? 'REMOTO' : 'PRESENCIAL',
      })
    ...
    if (action === 'RESCHEDULE') {
      ...
      const rescheduled = await rescheduleAppointment({
        ...
        startsAt: startsAt.toISOString(),
        endsAt: endsAt.toISOString(),
        mode: request.body.mode === 'REMOTO' ? 'REMOTO' : request.body.mode === 'PRESENCIAL' ? 'PRESENCIAL' : undefined,
      })
